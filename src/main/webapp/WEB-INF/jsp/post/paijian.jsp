           ...
           <c:forEach items="${paijians}" var="paijian">
                ...
                <td>${restate}</td>
                 <td>
                    ...
                    <%}else {%>
                    <c:if test="${state=='0'}">
                       <c:if test="${paijian.se_num<3}">
                        <a href="/post/zeri?se_id=${paijian.se_id}&area_id=${paijian.se_stateaddress.area_id}" disabled="true">
                            <span></span>择日派送
                        </a>
                       </c:if>
                       <c:if test="${paijian.se_num==3}">
                    ...
                    <c:if test="${state=='1'}">
                        <a href="/post/Status?st_id=2&se_id=${paijian.se_id}">
                            <span></span>正在派件
                        </a>
                        <a href="/post/Status?st_id=4&se_id=${paijian.se_id}">
                            <span></span>派件失败
                        </a>
                    </c:if>
                    <c:if test="${state=='2'}">
                        <a href="/post/Status?st_id=3&se_id=${paijian.se_id}">
                            <span></span>派件完成
                        </a>
                        <a href="/post/Status?st_id=5&se_id=${paijian.se_id}">
                            <span></span>派件失败
                        </a>
                    </c:if>
                    <c:if test="${state=='3'}">
